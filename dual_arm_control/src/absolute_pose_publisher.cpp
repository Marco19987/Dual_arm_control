...
public:
  AbsolutePosePublisher() : Node("absolute_pose_publisher")
  {
    ...
    publisher_fake_object_pose_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/ekf/object_pose", 1);
...
private:
  ...
  void timerCallback()
  {
    if (pose_msg_1_ && pose_msg_2_)
    {
      ...
      // publish fake pose
      Eigen::Matrix<double, 4, 4> aTfo;  // fake object pose in the absolute frame
      aTfo.setIdentity();
      aTfo.block<3, 1>(0, 3) << 0.1, 0, 0;

      Eigen::Matrix<double, 4, 4> bTa;  // absolute pose of robot 1 in the base frame
      pose_msg_to_eigen_matrix(new_pose_msg.pose, bTa);
      Eigen::Matrix<double, 4, 4> bTfo = bTa * aTfo;

      geometry_msgs::msg::PoseStamped fake_object_pose_msg;
      fake_object_pose_msg.header.stamp = this->now();
      fake_object_pose_msg.header.frame_id = "world";
      eigen_matrix_to_pose_msg(bTfo, fake_object_pose_msg.pose);
      publisher_fake_object_pose_->publish(fake_object_pose_msg);
    }
    else
    {
      // publish fake object pose
      geometry_msgs::msg::PoseStamped fake_object_pose_msg;
      fake_object_pose_msg.header.stamp = this->now();
      fake_object_pose_msg.header.frame_id = "world";
      fake_object_pose_msg.pose.position.x = 0.8;
      fake_object_pose_msg.pose.position.y = 0;
      fake_object_pose_msg.pose.position.z = 0.6;
      fake_object_pose_msg.pose.orientation.w = 0;
      fake_object_pose_msg.pose.orientation.x = 0;
      fake_object_pose_msg.pose.orientation.y = 0.7;
      fake_object_pose_msg.pose.orientation.z = -0.7;
      publisher_fake_object_pose_->publish(fake_object_pose_msg);
  ...
  rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr publisher_fake_object_pose_;
