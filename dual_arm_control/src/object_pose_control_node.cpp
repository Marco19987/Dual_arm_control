// thid node reads the acutal and the desired object pose and publish the control action (The absolute velocity)
...
{
public:
  ObjectPoseControlNode() : Node("object_pose_control")
  {
    // declare parameters
    ...
    control_gain_matrix_.diagonal() << control_gain_diag_vector[0], control_gain_diag_vector[1], control_gain_diag_vector[2],
        control_gain_diag_vector[3], control_gain_diag_vector[4], control_gain_diag_vector[5];
...
private:
  void timerCallback()
  {
    if (object_pose_read_ && desired_object_pose_read_ && absolute_pose_read_)
    {
      ...
      absolute_twist_.block<3, 1>(0, 0) = control_gain_matrix_.block<3, 3>(0, 0) * (bTa_d.block<3, 1>(0, 3) - bTa_.block<3, 1>(0, 3));
