...
Bm = blkdiag(M,I);
viscous_friction = 0.1*blkdiag(eye(3)*0.1, eye(3)*0.1); % viscous friction object-air
n_pose_measures = 6;
bg = [0;0;-9.8*0];
opg1 = [0.06, -0.025, -0.0]';
...
bTb2 = [quat2rotm([0.707,0.707,-0.0,-0.0]) [1.63;-0.00; 0.0]; 0 0 0 1];
...
W_k = eye(sizeState) * 1e-11; % Updated covariance noise matrix for state transition
...
% measure_occlusion = round(rand(2*n_pose_measures, numSteps+1))*1;
